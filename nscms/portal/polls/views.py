...
def vote(request, choice_id=None, template="cemese/portal/polls/question_result.html"):
    ...
    return object_detail(request=request,
                         ...
                         extra_context={
                            "choices": [{'object': i, 'percent': ("%.1f" % (i.votes / votes * 100)).zfill(4)} for i in choice.question.choice_set.order_by('-votes')]#Choice.objects.filter(question=)
                         })
